...
pub fn find_user_by_id(pool: &DatabasePool, uid: &Uuid) -> Result<User, ServError> {
    // use crate::schema::users::{self, dsl::*};
    let conn = pool.get()?;
    users
        .find(uid)
...
pub fn find_by_email(pool: &DatabasePool, em: &str, pa: &str) -> Result<User, ServError> {
    // use crate::schema::users::{self, dsl::*};
    let conn = pool.get()?;
...
pub fn update_user(pool: &DatabasePool, uid: Uuid, item: &UserChange) -> Result<User, ServError> {
    let conn = pool.get()?;
    let _user = users
        .find(uid)
        .get_result::<User>(&conn)
